...
from .networks import Softmax, FFN, Softmax, FFN_Params_Normals, FFN_Q
from .helpers import state_to_sarsa, apply_funcs, to_torch, toggle
from .distributions import lognorm_sample, lognorm_logprob, reg_policy
...
class LearningAgent(StateAgentWithWallet):
    ...
    def __init__(self, state_dim: int, action_cont_dim: int, device: str, action_discrete_dim: int, discount_factor: float, device: str):

        self.memory = defaultdict(list)
        self.memory_capacity = 10000

        # Q func
        self.q_func = FFN_Q(state_dim = state_dim, action_cont_dim = action_cont_dim)
        ...
        self.policy_discr = FFN(sizes = [state_dim, 32, action_discrete_dim], activation=nn.Tanh, output_activation = Softmax) # this network decides whether to buy/sell/do nothing
    ...
    def _update_memory(self, state: MarketState, action: Action, reward: float, next_state: MarketState):
        """
        Ensure what is the input?
        """
        self.memory['state'].append(state.to_array())
        # action_discrete = 0 if buy
        # action_discrete = 1 if sell
    ...
    def update_memory(self, states: List[Tuple[MarketState, Action]]):
        ...
        for key, value in self.memory.items():
            if len(self.memory[key]>self.memory_capacity):
                first_n = self.memory[key] - self.memory_capacity
    ...
    def create_dataloader(self, batch_size):
        ...
        dataset_state = apply_funcs(value = self.memory['state'], *(np.stack, to_torch_device))
        dataset_action_discrete = apply_funcs(value = self.memory['action_discrete'], *(np.array, partial(to_torch, dtype=torch.int64, device=self.device)))
        dataset_action_volume = apply_funcs(value = self.memory['action_volume'], *(np.array, to_torch_device))
        dataset_reward = apply_funcs(value = self.memory['reward'], *(np.array, to_torch_device))
        dataset_next_tate = apply_funcs(value = self.memory['next_state'], *(np.stack, to_torch_device))
    ...
    def state(self, vega: VegaServiceNull) -> MArketState:
        ...
        book_state = self.vega.market_depth(self.market_id, num_levels=5) # make num_levels as a parameter?
    ...
    def _step(self, vega_state: MarketState, random: bool = True) -> Action:
        if random:
            # random policy
    ...
    def sample_action(self, state: torch.Tensor, sim: bool = True, evaluate: bool = False):
        """
        Sample an action. 
    ...
    def policy_eval(self, batch_size: int, n_epochs: int,):
        ...
        for epoch in range(n_epochs):
            for i, (batch_state, batch_action_discrete, batch_action_volume, batch_reward, batch_next_state) in enumerate(dataloader):
                ...
                pred = torch.gather(self.q_func(batch_state, volume_sell, volume_buy), dim=1, index=batch_action_discrete)
                with torch.no_grad():
    ...
    def D_KL(self, state, n_mc):
        ...
        # I force the parameters of the lognormal not to be very far away from (0,1)
