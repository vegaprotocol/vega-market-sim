...
class SettlementDataSubmitter(StateAgentWithWallet):
    """
    Submits pre-supplied oracle data every at every step
    """

    NAME_BASE = "oracle_data_submitter"

    def __init__(
        self,
        key_name: str,
        oracle_data_generator: Iterable[float],
        market_name: Optional[str] = None,
        tag: str = "",
        wallet_name: Optional[str] = None,
    ):
        ...
        self.current_step = 0
        self.market_name = market_name
        self.oracle_data_generator = oracle_data_generator
    ...
    def step(self, vega_state: VegaState):
        ...
        self.curr_data = next(self.oracle_data_generator)

        # depending on order in which agents are traversed the market may not be initialised yet
        if self.market_id is None:
            id = self.vega.find_market_id(name=self.market_name)
            if id is None:
                return
            self.market_id = id
        ...
        self.vega.submit_settlement_data(
            ...
            settlement_price=self.curr_data,
            market_id=self.market_id,
        )
