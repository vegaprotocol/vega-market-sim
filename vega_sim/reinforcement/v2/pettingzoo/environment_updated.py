...
class MultiAgentVegaEnv(ParallelEnv):
    ...
    def __init__(
        ...
        self.current_step = 0
        self.trade_volume_levels = trade_volume_levels
        self.learner_names = {
    ...
    def _get_action_space(self, action_type: ActionType) -> spaces.Space:
        if action_type == ActionType.MARKET:
            self.num_actions_side =3
        elif action_type == ActionType.AT_TOUCH_ONE_SIDE:
            self.num_actions_side =2
        else:
            self.num_actions_side =0
            raise Exception(f"Action type {action_type} is not implemented")
        
        return spaces.Tuple((spaces.Discrete(self.num_actions_side), spaces.Discrete(self.trade_volume_levels)))
    ...
    def _convert_one_dimensional_action(self, one_dim_action):
        action_type = one_dim_action // self.trade_volume_levels
        trade_volume_level = one_dim_action % self.trade_volume_levels
        return (action_type, trade_volume_level)
    ...
    def step(self, actions):
        ...
        for agent_id, agent_name in self.learner_names.items():
            ...
            if self.is_test==True: self.record_dict[agent_name]+=reward
        ...
        if is_terminal and self.is_test:print(self.record_dict)
