...
from configs import agent_configs,get_agents, common_params_dict
...
def train(max_epoch=200,step_per_epoch=500,step_per_collect=2,episode_per_test=9,batch_size=64,update_per_step=0.05,
    ...
    try:
        ...
        def save_best_fn(policy):
            ...
            optim_state_dicts = {agent_name: {'actor_opt': agent_policy.actor_opt.state_dict(), 'critic1_opt': agent_policy.critic1_opt.state_dict(), 'critic2_opt': agent_policy.critic2_opt.state_dict()} for agent_name, agent_policy in policy.policies.items()}
            torch.save(
                {
        ...
        def save_checkpoint_fn(epoch, env_step):
        ...
        try:
            ...
            for agent_name, agent_state_dict in agent_state_dicts.items():
                if isinstance(policy.policies[agent_name], DQNPolicy):
            ...
            for agent_name, optim_state_dict in optim_state_dicts.items():
                policy.policies[agent_name].optim.load_state_dict(optim_state_dict)
            print("Optim loaded")
        ...
        def epsilon_decay(step,EPS_DECAY=0.05):
        ...
        train_collector.collect(n_episode=n_train_collector_init)
...
if __name__ == "__main__":
    train(max_epoch=300,
          step_per_epoch=1000,step_per_collect=5,episode_per_test=5,
          batch_size=128,update_per_step=0.01, n_replay=10000,n_train_collector_init=15,
          load_model=not True,model_name='checkpoint.pth', resume=not True,
          **common_params_dict
        )
'''
    train(max_epoch=10,
          step_per_epoch=100,step_per_collect=5,episode_per_test=1,
          batch_size=10,update_per_step=0.05, n_replay=100,n_train_collector_init=2,
          load_model=not True,model_name='checkpoint_dummy.pth', resume=not True,
          **common_params_dict)
'''